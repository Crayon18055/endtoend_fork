...
def test_model(data_dir, checkpoint_path=None):
    ...
    print("pictures loaded from", small_data_dir)
    ...
    for i, image_path in enumerate(selected_images):
        ...
        # 设置 trg 为 [1, 0]
        if target_row.empty:
            print(f"Warning: No target data found for image {image_name}.")
            trg_text = "trg: N/A"
        else:
            trg_vector = target_row.iloc[0, [4, 5]].values.astype(float)
